 ...
 *
 * Ported from `@router-for-me/pi-cliproxyapi-provider` (MIT) with harness-neutral
 * shapes: this module imports nothing from the omp runtime so it stays unit
...
/** A single model definition accepted by `pi.registerProvider()`. */
...
export const CLIENT_VERSION = "omp";
...
export const ZERO_COST: ProviderCost = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 };
...
/** Client version CPA logs and gates on; overridable for deployments that reject the default. */
export function clientVersion(): string {
	const override = process.env.CLIPROXYAPI_CLIENT_VERSION?.trim();
	return override || CLIENT_VERSION;
}
 ...
 * - models always at {root}/v1/models?client_version=omp
 */
export function resolveEndpoints(baseUrlInput: string): {
	...
	const modelsUrl = `${url.origin}${modelsPath}?client_version=${encodeURIComponent(clientVersion())}`;
 ...
 * Priority: env > cliproxyapi.json > /login credential store > default baseUrl
 */
export function resolveConnection(
	agentDir: string,
	stored: { baseUrl?: string; apiKey?: string } | null = null,
	...
	const baseUrlInput = firstNonEmpty(process.env.CLIPROXYAPI_BASE_URL, file.baseUrl, stored?.baseUrl, DEFAULT_BASE_URL)!;
	const apiKey = firstNonEmpty(process.env.CLIPROXYAPI_API_KEY, file.apiKey, stored?.apiKey);
	if (!apiKey) {
		return null;
	}
...
/**
 * Map CPA reasoning levels onto omp's `ThinkingConfig`.
 *
 * omp's effort ladder stops at `max`; CPA's `ultra` has no omp equivalent and is
 * dropped. An empty result means the model's effort cannot be steered, which omp
 * encodes as `thinking: undefined` rather than an empty ladder.
 */
export function buildThinking(efforts: string[]): ThinkingConfig | undefined {
	if (efforts.length === 0) {
		return undefined;
	}

	const supported = new Set(efforts.map((effort) => effort.trim().toLowerCase()));
	const mapped = THINKING_EFFORTS.filter((effort) => supported.has(effort));
	if (mapped.length === 0) {
		return undefined;
	}

	return { mode: "effort", efforts: mapped };
}
...
export function toOmpModel(
	...
	return {
		...
		thinking: buildThinking(efforts),
...
function parseModelsDevCost(raw: ModelsDevCostPayload | undefined): ModelsDevCost | undefined {
...
function cloneCost(cost: ModelsDevCost): ModelsDevCost {
	return {
		input: cost.input,
		output: cost.output,
...
function getModelsDevCachePath(agentDir?: string): string {
	...
	return join(tmpdir(), "omp-cliproxyapi-models-dev-cache.json");
}
...
/** Fetch/pass-through catalog entries keep their fast tiers out of omp's cost card. */
export function matchModelCost(modelId: string, costCatalog: ModelsDevCostCatalog, isFastMode = false): ProviderCost {
...
export async function loadMappedModels(
	...
	const models = remoteModels
		.map((model) => toOmpModel(model, costCatalog, effectiveFastMode))
		.filter((model): model is OmpProviderModel => model !== null);
